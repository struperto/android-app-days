...
class AreaAuthoringProjectorTest {
    ...
    fun projector_mapsSeededAreaIntoAuthoringSections() {
        ...
        assertEquals(
            listOf("Zustand", "Eingang", "Auswahl", "Rhythmus"),
    ...
    fun projector_respectsDefinitionAuthoringAxesWhenBuildingSections() {
        ...
        assertEquals(listOf("Zustand", "Auswahl"), state.sections.first().axes.map { it.label })
        assertEquals(listOf("Sichtbarkeit"), state.sections[1].axes.map { it.label })
        assertEquals(listOf("Zustand", "Auswahl"), state.previewAxes.map { it.label })
        assertTrue(state.summary.contains("Zustand"))
        assertTrue(state.summary.contains("Auswahl"))
    }
}
