...
private fun stateStatusPresentation(
    ...
    return StartResolvedStatusPresentation(
        ...
        panelSummary = stateOption?.let { "Stand ${it.label}" }
        ...
        coreTitle = statusSemantics?.stateCoreTitle ?: "Aktueller Zustand",
        coreValue = stateOption?.label ?: "offen",
        coreCaption = optionSummary,
        selectionActionId = StartPanelActionId.SnapshotState,
        selectionActionLabel = "Stand",
        selectionValueLabel = stateOption?.label ?: "offen",
        selectionSheetTitle = "Stand",
        selectionOptions = listOf(
            StartPanelOptionState(
                id = "clear",
                label = statusSemantics?.stateResetLabel ?: "Ohne Eintrag",
                selected = stateOption == null,
                supportingLabel = statusSemantics?.stateResetSummary ?: "ohne frischen Zustand",
            ),
        ...
        primaryMetric = StartPanelMetricState(
            statusSemantics?.stateMetricLabel ?: "Vertrauen",
            statusSemantics?.stateMetricValue ?: "Reflexion",
        ),
        resetValueLabel = if (stateOption == null) "offen" else "setzen",
    )
}
...
private fun scoreStatusPresentation(
    ...
    val detailPrefix = pilotPrefix ?: "Wert"
    ...
    return StartResolvedStatusPresentation(
        ...
        panelSummary = manualScore?.let { "Stand $it/5" } ?: "Stand offen",
        ...
        infoLabel = "Stand",
        coreTitle = scoreCoreTitle ?: pilotPrefix?.let { "$it aktuell" } ?: "Aktueller Wert",
        coreValue = manualScore?.let { "$it/5" } ?: "offen",
        coreCaption = manualScore?.let(::scoreOptionSupportingLabel) ?: "ohne frischen Wert",
        selectionActionId = StartPanelActionId.SnapshotScore,
        selectionActionLabel = "Stand",
        selectionValueLabel = manualScore?.let { "$it/5" } ?: "offen",
        selectionSheetTitle = if (pilotPrefix != null) "$pilotPrefix setzen" else "Stand",
        selectionOptions = listOf(
            StartPanelOptionState(
                id = "clear",
                label = "Offen",
                selected = manualScore == null,
                supportingLabel = "ohne frischen Wert",
            ),
        ) + (1..5).map { score ->
            StartPanelOptionState(
                id = score.toString(),
...
private fun pilotScorePrefix(
    ...
    return when (definition.overviewMode) {
        ...
        AreaOverviewMode.HYBRID -> "Status"
    }
}
