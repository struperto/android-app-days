...
fun projectAreaAuthoringStudioState(
    ...
    val sections = buildList {
        if (characterAxes.isNotEmpty()) {
            add(
                AreaAuthoringSectionState(
                    title = "Charakterachsen",
                    summary = "Wie der Bereich Lage liest und Richtung, Quellen und Flow rahmt.",
                    axes = characterAxes,
                ),
            )
        }
        if (depthAxes.isNotEmpty()) {
            add(
                AreaAuthoringSectionState(
                    title = "Studio-Tiefe",
                    summary = "Wie viel Charaktertiefe und Sichtbarkeit dieser Bereich im Studio bekommt.",
                    axes = depthAxes,
                ),
            )
        }
    ...
    val basisSummary = when {
        definition != null && definition.seededByDefault -> "${definition.title} bleibt die Typbasis, der Charakter ist jetzt authorierbar."
        else -> "${template.label} liefert die Ausgangsform, die Charakterachsen formen den Bereich weiter aus."
    }
...
private fun buildAuthoringSummary(
    ...
    val fragments = buildList {
        if (axes.any { it.axis == AreaAuthoringAxis.STATUS_SCHEMA }) {
            add("Liest ${authoringLageSummary(authoring.lageMode)}")
        }
        if (axes.any { it.axis == AreaAuthoringAxis.DIRECTION }) {
            add("richtet ${authoringDirectionSummary(authoring.directionMode, primaryTracks)}")
        }
        if (axes.any { it.axis == AreaAuthoringAxis.SOURCES }) {
            add("nutzt ${authoringSourcesSummary(authoring.sourcesMode)}")
        }
        if (axes.any { it.axis == AreaAuthoringAxis.FLOW }) {
            add("bleibt ${authoringFlowSummary(authoring.flowProfile)}")
        }
    }
    return when {
        fragments.isEmpty() -> "Charakter und Studiotiefe bleiben fuer diesen Bereich anpassbar."
...
private fun authoringLageSummary(lageMode: AreaLageMode): String {
    return when (lageMode) {
        AreaLageMode.Score -> "eher ueber Wert und Zielabstand"
        AreaLageMode.State -> "eher ueber Zustand und Lesepunkt"
    }
}
...
private fun authoringDirectionSummary(
    ...
    return when (directionMode) {
        AreaDirectionMode.Balanced -> "Fokus und Takt gemeinsam"
        AreaDirectionMode.Focus -> "$leadTrack klar nach vorn"
        AreaDirectionMode.Rhythm -> "staerker ueber den Takt"
    }
}

private fun authoringSourcesSummary(sourcesMode: AreaSourcesMode): String {
    return when (sourcesMode) {
        AreaSourcesMode.Balanced -> "offene Spuren"
        AreaSourcesMode.Signals -> "signalnahe Hinweise"
        AreaSourcesMode.Curated -> "gezielte kuratierte Quellen"
    }
}

private fun authoringFlowSummary(flowProfile: AreaFlowProfile): String {
    return when (flowProfile) {
        AreaFlowProfile.Stable -> "ruhig und lokal"
        AreaFlowProfile.Supportive -> "tragend und rueckkehrfaehig"
        AreaFlowProfile.Active -> "aktiver und zugorientiert"
    }
}
...
internal fun areaAuthoringSourcesOptions(selected: AreaSourcesMode): List<AreaAuthoringOptionState> {
    return AreaSourcesMode.entries.map { mode ->
        AreaAuthoringOptionState(
            ...
            selected = selected == mode,
            supportingLabel = when (mode) {
                AreaSourcesMode.Balanced -> "laesst mehrere Spuren zusammenstehen"
                AreaSourcesMode.Signals -> "zieht Signale und Frische staerker nach vorn"
                AreaSourcesMode.Curated -> "betont gezielte und bewusste Quellen"
            },
        )
    }
}
