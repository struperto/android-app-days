...
import com.struperto.androidappdays.domain.area.AreaRuntimeContract
...
import com.struperto.androidappdays.domain.area.buildAreaRuntimeContract
...
data class StartOverviewState(
    ...
    val pendingImports: List<StartPendingImportState> = emptyList(),
    val capabilityProfile: CapabilityProfile? = null,
    val activeAreaCount: Int = 0,
    val maxActiveAreaCount: Int = 16,
)

data class StartPendingImportState(
    val id: String,
    val kind: AreaImportKind,
    val title: String,
    val detail: String,
    val reference: String,
    val suggestedInput: String,
)
...
data class StartAreaDetailState(
    ...
    val todayOutput: AreaTodayOutput,
    val runtimeContract: AreaRuntimeContract = AreaRuntimeContract(),
)
    ...
    return StartOverviewState(
        areas = orderedAreas
            .mapIndexed { index, input ->
                ...
                val selectedTracks = normalizeSelectedTracks(
                    tracks = tracks,
...
fun projectStartAreaDetailState(
    ...
    return StartAreaDetailState(
        ...
        todayOutput = todayOutput,
        runtimeContract = buildAreaRuntimeContract(
            definition = effectiveDefinition,
            blueprint = input.blueprint,
        ),
    )
...
private fun resolveStartAreaFamily(
    ...
    return when {
        instance.templateId == "medium" || instance.templateId == "theme" ||
            containsAnyWordPrefix("news", "radar", "artikel", "quelle", "quellen", "website", "feed", "nachrichten", "lesen", "podcast") -> {
...
fun startAreaSnapshotLabel(
    ...
    return actualScore?.let { "Stand $it/5" } ?: "Stand offen"
}
    ...
    return when (panel) {
        StartAreaPanel.Snapshot -> StartPanelScreenState(
            ...
            core = StartPanelCoreState(
                ...
                metrics = listOf(
                    ...
                    StartPanelMetricState("Schema", profileState.lageLabel),
        ...
        StartAreaPanel.Path -> {
            ...
            StartPanelScreenState(
                ...
                metrics = listOf(
                    StartPanelMetricState("Fokus", focusTrack),
                    StartPanelMetricState("Regel", profileState.directionLabel),
                    StartPanelMetricState("Fenster", cadenceLabel(cadence)),
                ),
                actions = listOf(
                    StartPanelActionState(
                        ...
                        sheet = StartPanelSheetState(
                            ...
                            options = listOf(
                                StartPanelOptionState(
                                    ...
                                    supportingLabel = "holt heute den aktuellen Ausschnitt nach vorn",
                                ),
                                StartPanelOptionState(
                                    id = "weekly",
                                    label = "Woechentlich",
                                    selected = cadence == "weekly",
                                    supportingLabel = "haelt den Bereich ueber Wochen statt nur ueber heute offen",
                                ),
                                StartPanelOptionState(
                                    id = "adaptive",
                                    label = "Adaptiv",
                                    selected = cadence == "adaptive",
                                    supportingLabel = "passt den Ausschnitt an Zustand und Eingang an",
                                ),
                            ),
                        ),
                    ),
        ...
        StartAreaPanel.Sources -> {
            ...
            StartPanelScreenState(
                ...
                core = StartPanelCoreState(
                    title = sourceSemantics?.coreTitle ?: "Aktives Material",
                ...
                actions = listOf(
                    ...
                    StartPanelActionState(
                        ...
                        valueLabel = when (selectedTracks.size) {
                            1 -> "1 aktiv"
                            else -> "${selectedTracks.size} aktiv"
                        },
                        mode = StartPanelActionMode.Sheet,
                        sheet = StartPanelSheetState(
                            title = sourceSemantics?.coreTitle ?: "Eingang",
        ...
        StartAreaPanel.Options -> {
            ...
            StartPanelScreenState(
                ...
                core = StartPanelCoreState(
                    title = flowSemantics?.coreTitle ?: "Lokaler Takt",
                ...
                metrics = listOf(
                    ...
                    StartPanelMetricState("Tempo", "$intensity/5"),
                ),
                actions = listOf(
                    StartPanelActionState(
                    ...
                    StartPanelActionState(
                        ...
                        mode = StartPanelActionMode.Sheet,
                        sheet = StartPanelSheetState(
                            title = startAreaFlowActionLabel(kernelBlueprint),
                            selectionMode = StartPanelSelectionMode.Multiple,
...
fun startAreaFocusTrack(
    ...
    return tracks.firstOrNull { it in selectedTracks }
        ?: tracks.firstOrNull()
        ?: "Spur"
}

private fun normalizeSelectedTracks(
    tracks: List<String>,
    selectedTracks: Set<String>,
): Set<String> {
    val normalized = selectedTracks.filterTo(linkedSetOf()) { track -> track in tracks }
    return normalized.ifEmpty {
        tracks.take(2).toCollection(linkedSetOf())
    }
}
...
private fun startAreaSourcesEffectLabel(
    ...
    val orderedTracks = trackLabels.filter { it in selectedTracks }
    if (orderedTracks.isEmpty()) {
        return "Noch keine aktive Spur fuer den Eingang."
    }
    val leadTrack = orderedTracks.first()
    return when (sourcesMode) {
        AreaSourcesMode.Balanced -> when (orderedTracks.size) {
            1 -> "$leadTrack formt den Eingang gerade allein."
            2 -> "${orderedTracks[0]} und ${orderedTracks[1]} formen den Eingang gemeinsam."
            else -> "${orderedTracks[0]} und ${orderedTracks[1]} fuehren den Eingang, ${orderedTracks.size - 2} weitere laufen mit."
        }

        AreaSourcesMode.Curated -> "$leadTrack fuehrt den Eingang."

        AreaSourcesMode.Signals -> when {
            signalBlend >= 75 -> "Signale rund um $leadTrack fuehren den Eingang deutlich."
            signalBlend >= 50 -> "Signale zu $leadTrack mischen stark mit."
            signalBlend > 0 -> "$leadTrack bleibt vorn, Signale mischen leicht mit."
            else -> "$leadTrack fuehrt ohne Signalanteil."
        }
    }
}
...
private fun startAreaSnapshotEffectLabel(
    ...
    return when (lageMode) {
        AreaLageMode.Score -> if (hasValue) {
            "Der Wert zeigt direkt die Distanz zum Soll."
        } else {
            "Ohne Wert bleibt nur das Soll als Orientierung."
        }

        AreaLageMode.State -> if (hasValue) {
            "Der Zustand legt fest, wie der Bereich gelesen wird."
        } else {
            "Ohne Zustand bleibt die Einordnung offen."
        }
    }
}
...
private fun startAreaDirectionEffectLabel(
    ...
    return when (directionMode) {
        AreaDirectionMode.Focus -> "$focusTrack steht im Fokus."
        AreaDirectionMode.Rhythm -> when (cadence) {
            "daily" -> "$focusTrack fuehrt den ersten Schritt."
            "weekly" -> "Diese Woche fuehrt $focusTrack den Bereich."
            else -> "Der Bereich richtet sich zuerst auf $focusTrack aus."
        }

        AreaDirectionMode.Balanced -> when (cadence) {
            "daily" -> "$focusTrack steht zuerst, der Rest folgt im Takt."
            "weekly" -> "Diese Woche fuehrt $focusTrack, ohne den Rest auszublenden."
            else -> "$focusTrack fuehrt, waehrend der Rest mitlaeuft."
        }
    }
}
...
private fun startAreaProfileState(
    ...
    val directionLabel = when (profileConfig.directionMode) {
        ...
        AreaDirectionMode.Focus -> "Klar"
        AreaDirectionMode.Rhythm -> "Im Takt"
    }
...
private fun areaLageModeOptions(
    ...
    return listOf(
        ...
        StartPanelOptionState(
            ...
            supportingLabel = "liest den Bereich eher ueber Zustand und Einordnung",
        ),
    )
}
...
private fun areaDirectionModeOptions(
    ...
    return listOf(
        StartPanelOptionState(
            ...
            supportingLabel = "haelt mehrere Spuren gemeinsam sichtbar",
        ),
        StartPanelOptionState(
        ...
        StartPanelOptionState(
            ...
            supportingLabel = "liest Auswahl staerker ueber das Zeitfenster",
        ),
    )
}
...
private fun sourcesBlendSupportingLabel(
    ...
    return when (value) {
        ...
        75 -> "Signale ziehen die Auswahl deutlich"
...
private fun startAreaFlowEffectLabel(
    ...
    return when (flowProfile) {
        AreaFlowProfile.Stable -> toggleLead?.let {
            "$it haelt die Rueckkopplung $intensityCue ruhig."
        } ?: "Die Rueckkopplung bleibt ruhig und lokal."

        AreaFlowProfile.Supportive -> toggleLead?.let {
            "$it stuetzt den Bereich $intensityCue in der Rueckkehr."
        } ?: "Der Flow stuetzt still im Hintergrund."

        AreaFlowProfile.Active -> toggleLead?.let {
            "$it zieht den Bereich $intensityCue nach vorn."
        } ?: "Zug ist angelegt, aber noch ohne aktive Impulse."
    }
}
