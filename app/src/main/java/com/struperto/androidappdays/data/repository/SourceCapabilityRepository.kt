                ...
                when (source) {
                            ...
                            detail = when (healthAvailability) {
                                HealthConnectAvailability.AVAILABLE -> {
                                    if (healthPermissions.containsAll(healthConnectRepository.requiredPermissions)) {
                                        "Schlaf- und Bewegungsdaten werden fuer den Tag eingelesen."
                                    } else {
                                        "Die Quelle ist verfuegbar, aber noch nicht fuer Schlaf und Bewegung freigegeben."
                                    }
                                }
                                HealthConnectAvailability.UPDATE_REQUIRED -> "Health Connect braucht vor der Nutzung ein Update."
                                HealthConnectAvailability.UNAVAILABLE -> "Health Connect ist auf diesem Geraet nicht verfuegbar."
                            },
                        )
                    }
                    DataSourceKind.CALENDAR -> {
                        ...
                        DataSourceCapability(
                            ...
                            granted = granted,
                            detail = if (granted) {
                                "Termindruck und Fokusfenster koennen als Kontextsignal einflieBen."
                            } else {
                                "Der Kalender ist optional und noch nicht fuer Kontextsignale freigegeben."
                            },
                        )
                    }
                    DataSourceKind.NOTIFICATIONS -> {
                        ...
                        DataSourceCapability(
                            ...
                            granted = granted,
                            detail = if (granted) {
                                "Unterbrechungsdruck kann direkt aus dem Notification Listener gelesen werden."
                            } else {
                                "Der Notification Listener ist noch nicht aktiv."
                            },
                        )
                    }
                    DataSourceKind.MANUAL -> {
                        DataSourceCapability(
                            ...
                            available = true,
                            granted = true,
                            detail = "Manuelle Eintraege bleiben der direkte Rueckkanal fuer Werte ohne passive Quelle.",
                        )
                    }
                }
            },
