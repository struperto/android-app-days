...
val MIGRATION_14_15 = object : Migration(14, 15) {
    override fun migrate(db: SupportSQLiteDatabase) {
        db.execSQL(
            """
            CREATE TABLE IF NOT EXISTS `area_web_feed_sources` (
                `areaId` TEXT NOT NULL,
                `url` TEXT NOT NULL,
                ...
                `lastStatusLabel` TEXT NOT NULL,
                `lastStatusDetail` TEXT NOT NULL,
                PRIMARY KEY(`areaId`, `url`)
            )
            """.trimIndent(),
        )
        db.execSQL(
            """
            CREATE INDEX IF NOT EXISTS `index_area_web_feed_sources_areaId`
            ON `area_web_feed_sources` (`areaId`)
            """.trimIndent(),
        )
    }
}
...
val MIGRATION_15_16 = object : Migration(15, 16) {
    override fun migrate(db: SupportSQLiteDatabase) {
        db.execSQL(
            """
            ALTER TABLE `area_web_feed_sources`
            ADD COLUMN `sourceKind` TEXT NOT NULL DEFAULT 'website'
            """.trimIndent(),
        )
        db.execSQL(
            """
            ALTER TABLE `area_web_feed_sources`
            ADD COLUMN `syncCadence` TEXT NOT NULL DEFAULT '6h'
            """.trimIndent(),
        )
        db.execSQL(
            """
            UPDATE `area_web_feed_sources`
            SET `sourceKind` = CASE
                WHEN lower(`url`) LIKE '%.xml'
